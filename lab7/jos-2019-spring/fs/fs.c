...
int alloc_block(void)
{
	...
	uint32_t total_block_num = super->s_nblocks;
	for (uint32_t block_index = 0; block_index < total_block_num; block_index++)
	{
		if (block_is_free(block_index))
		{
			bitmap[block_index / 32] &= ~(1 << (block_index % 32));
			flush_block(&bitmap[block_index / 32]);
			return block_index;
		}
	}
