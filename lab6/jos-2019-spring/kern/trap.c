...
void trap_init(void)
{
	...
	SETGATE(idt[T_DEBUG], 0, GD_KT, T_DEBUG_ENTRY, 0);
	SETGATE(idt[T_NMI], 0, GD_KT, T_NMI_ENTRY, 0);
	SETGATE(idt[T_BRKPT], 0, GD_KT, T_BRKPT_ENTRY, 3);
	SETGATE(idt[T_OFLOW], 0, GD_KT, T_OFLOW_ENTRY, 0);
	SETGATE(idt[T_BOUND], 0, GD_KT, T_BOUND_ENTRY, 0);
...
void page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall)
	{
		...
		{
			utf = (struct UTrapframe *)(curenv->env_tf.tf_esp - sizeof(void *) - sizeof(struct UTrapframe));
		}
		else
		{
